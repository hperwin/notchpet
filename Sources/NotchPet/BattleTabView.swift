...
final class BattleTabView: DSTabView {
    ...
    private func buildArena(movesEnabled: Bool) {
        ...
        // Opponent HP text
        let oppHPText = DS.label("HP: \(st.opponentPokemon.currentHP)/\(st.opponentPokemon.stats.maxHP)",
        ...
        // Player HP text
        let playerHPText = DS.label("HP: \(st.playerPokemon.currentHP)/\(st.playerPokemon.stats.maxHP)",
                                    size: 9, bold: false, color: DS.textSecondary)
